...
const DEFAULT_YEAR_BADGES: UserBadge[] = [
  { id: 'b-1', name: 'Jan - New Dawn', icon: '🔥', description: 'Log 20 active days in January', unlockedAt: '2026-01-31' },
  { id: 'b-2', name: 'Feb - Heart & Iron', icon: '❤️', description: 'Complete 15 high-intensity sessions', unlockedAt: '2026-02-28' },
  { id: 'b-3', name: 'Mar - Spring Surge', icon: '🌱', description: 'Log 10,000 steps for 14 straight days', unlockedAt: '2026-03-30' },
  { id: 'b-4', name: 'Apr - Protein Mastery', icon: '🥩', description: 'Hit daily protein target 25 times', unlockedAt: '2026-04-29' },
  { id: 'b-5', name: 'May - Recomp Peak', icon: '⚡', description: 'Achieve 2% body fat reduction', unlockedAt: '2026-05-31' },
  { id: 'b-6', name: 'Jun - Summer Shred', icon: '☀️', description: 'Hydration goal streak for 21 days', unlockedAt: '2026-06-25' },
  { id: 'b-7', name: 'Jul - Iron Will', icon: '🏋️', description: '50 completed workout sessions', unlockedAt: '2026-07-28' },
  { id: 'b-8', name: 'Aug - Consistency Titan', icon: '👑', description: 'Active 30 consecutive days', unlockedAt: '2026-08-05' },
  ...
  { id: 'b-m1', name: 'Hydration Hero', icon: '💧', description: 'Logged over 100 gallons of water', unlockedAt: '2026-06-15' },
  { id: 'b-m2', name: 'Centurion', icon: '💯', description: 'Logged 100 total days in Gym Hub', unlockedAt: '2026-07-10' },
  { id: 'b-m3', name: 'Macro Ninja', icon: '🥗', description: 'Hit exact macros 30 days in a row', unlockedAt: '2026-08-01' },
  { id: 'b-m4', name: 'Trainer Certified', icon: '⭐', description: 'Unlocked UNLCKD Pro Trainer Tier', unlockedAt: '2026-08-02' },
];
...
export const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  const [height, setHeight] = useState<number>(userProfile?.height || 70);
  const [heightUnit, setHeightUnit] = useState<'ftin' | 'cm'>(
    userProfile?.heightUnit === 'cm' ? 'cm' : 'ftin'
  );
  const [weight, setWeight] = useState<number>(userProfile?.weight || 185);
  const [weightUnit, setWeightUnit] = useState<'lbs' | 'kg'>(userProfile?.weightUnit || 'lbs');
  const [goalWeight, setGoalWeight] = useState<number>(userProfile?.goalWeight || 175);
  ...
  const [measurements, setMeasurements] = useState({
    ...
    calves: initialMeasurements.calves || 15,
  });
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <Card className="p-6 bg-brand-surface border-white/10 relative overflow-hidden">
      ...
      <Card className="p-6 bg-brand-surface border-white/10 space-y-6">
        <h3 className="text-xs font-black uppercase tracking-widest text-brand-primary flex items-center gap-2">
          <User className="w-4 h-4" /> Personal Metrics & Targets
        </h3>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <UnitToggle<'ftin' | 'cm'>
                ...
                value={heightUnit}
                onChange={(u) => {
                  if (u === 'cm' && heightUnit !== 'cm') setHeight(Math.round(height * 2.54));
                  if (u === 'ftin' && heightUnit === 'cm') setHeight(Math.round(height / 2.54));
                  setHeightUnit(u);
                }}
                size="sm"
              />
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="relative flex items-center">
                  <input
                    ...
                    max="8"
                    placeholder="5"
                    value={Math.floor((height || 0) / 12)}
                    onChange={(e) => {
                      const f = e.target.value === '' ? 0 : parseInt(e.target.value, 10);
                      const currentInches = (height || 0) % 12;
                      setHeight((isNaN(f) ? 0 : f) * 12 + currentInches);
                    }}
                ...
                <div className="relative flex items-center">
                  <input
                    ...
                    max="11"
                    placeholder="10"
                    value={Math.round((height || 0) % 12)}
                    onChange={(e) => {
                      const currentFeet = Math.floor((height || 0) / 12);
                      const i = e.target.value === '' ? 0 : parseInt(e.target.value, 10);
                      setHeight(currentFeet * 12 + (isNaN(i) ? 0 : i));
                    }}
                    className="w-full bg-white/5 border border-white/10 rounded-xl pl-3 pr-8 py-2 text-xs text-white font-mono focus:border-brand-primary outline-none"
                  />
      ...
      <Card className="p-6 bg-brand-surface border-white/10 space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-black uppercase tracking-widest text-emerald-400 flex items-center gap-2">
            <Ruler className="w-4 h-4" /> Body Measurements (9 Points)
          </h3>
          <UnitToggle<'in' | 'cm'>
            unitA="in"
            unitB="cm"
            ...
            onChange={(u) => {
              if (u === 'cm' && measurementLengthUnit === 'in') {
                setMeasurements(prev => ({
                  neck: Math.round(prev.neck * 2.54 * 10) / 10,
                  chest: Math.round(prev.chest * 2.54 * 10) / 10,
                  waist: Math.round(prev.waist * 2.54 * 10) / 10,
                  hips: Math.round(prev.hips * 2.54 * 10) / 10,
                  leftArm: Math.round(prev.leftArm * 2.54 * 10) / 10,
                  rightArm: Math.round(prev.rightArm * 2.54 * 10) / 10,
                  leftThigh: Math.round(prev.leftThigh * 2.54 * 10) / 10,
                  rightThigh: Math.round(prev.rightThigh * 2.54 * 10) / 10,
                  calves: Math.round(prev.calves * 2.54 * 10) / 10,
                }));
